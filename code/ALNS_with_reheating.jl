...
function plot_heartrate(accepted_costs::Vector{Float64}, best_costs::Vector{Float64}, dataset_id::String)
    iterations = 1:length(accepted_costs)
    plot(iterations, accepted_costs, label="Accepted", color=:green, lw=2)
    plot!(iterations, best_costs, label="Best", color=:red, lw=2)
    xlabel!("Iterations")
    ylabel!("Cost")
    title!("Overall Cost Evolution for $(dataset_id)")
end


function plot_temperature_evolution(temperature_evolution::Vector{Float64})
    iterations = 1:length(temperature_evolution)
    p = plot(
        iterations, temperature_evolution,
...
function ALNS_with_reheating(
    ...
    s2::Float64,                                  #
    s3::Float64,                                  #
    stagnation_threshold::Int64,
    reheat_threshold::Float64,
    reheat_factor::Float64
)

    # Initialize variables
    #stagnation_threshold = 1000  # num of iterations with no improvement
    #reheat_threshold = 1e-8      # min temperature before reheating (but there can still be reheating due to stagnation)
    #reheat_factor = 1.05         # Factor to reheat the temperature
    ...
    accepted_costs = Float64[]
    best_costs = Float64[]
    temperature_evolution = Float64[]  
    ...
    destruction_outcomes = [[0, 0, 0] for _ in 1:5]    # e.g. destruction_outcomes = [[s1, s2, s3], [s1, s2, s3], [s1, s2, s3], [s1, s2, s3], [s1, s2, s3]]
    ...
    # Define reward parameters (s) - these remain unchanged during all iterations
    ...
    while time() - start_time < time_limit
        ...
        iterations += 1
        push!(temperature_evolution, T)  

        #println("iteration is: ", iterations, " and temperature is: ", T, " and temperature evolution is: ", temperature_evolution)
...
=#
        ...
        if !(solution_tuple in visited_solutions)
            ...
            if new_cost_2swap < best_cost   
                ...
                # Log the global best outcome
                now_time = time() - start_time  # Calculate elapsed time
                push!(tracker.global_best, HeuristicOutcome(iterations, now_time, destruction_index, construction_index, new_cost_2swap))
                #println("New global best found with cost: $best_cost at iteration $iter and time $(round(now_time, digits=2)) seconds.")
            ...
            elseif new_cost_2swap < current_cost
                ...
                # Better solution found
                now_time = time() - start_time  # Calculate elapsed time
                push!(tracker.better, HeuristicOutcome(iterations, now_time, destruction_index, construction_index, new_cost_2swap))
                #println("Better unvisited solution accepted with cost: $new_cost_2swap.")
            else
                # Worse but unvisited solution accepted
                ...
                # Worse but unvisited solution accepted
                now_time = time() - start_time  # Calculate elapsed time
                push!(tracker.worse, HeuristicOutcome(iterations, now_time, destruction_index, construction_index, new_cost_2swap))
                #println("Worse unvisited solution accepted with cost: $new_cost_2swap.")
        ...
        push!(accepted_costs, current_cost)
        ...
        if iterations % segment_size == 0
            ...
            segment = iterations ÷ segment_size  # current segment number

            for i in eachindex(destruction_scores)   
                destruction_scores[i] = s1 * destruction_outcomes[i][1] + s2 * destruction_outcomes[i][2] + s3 * destruction_outcomes[i][3]
            end
            for i in eachindex(construction_scores) 
                construction_scores[i] = s1 * construction_outcomes[i][1] + s2 * construction_outcomes[i][2] + s3 * construction_outcomes[i][3]
            end
            ...
            push!(weight_evolution, WeightEvolution(iterations, deepcopy(destruction_weights), deepcopy(construction_weights)))
        ...
        # Step 4: Reheat logic
        if T < reheat_threshold || stagnation_counter >= stagnation_threshold
            println("Reheating at iteration $iterations: Temperature increased by a factor of $reheat_factor")
            T *= reheat_factor  
            stagnation_counter = 0  
        end
        ...
        push!(weight_evolution, WeightEvolution(iterations, deepcopy(destruction_weights), deepcopy(construction_weights)))   # Update every iteraton

        # Stop early if temperature is too low
        if T < 1e-8
            println("Temperature too low, stopping early at iteration $iterations")
            break
        end
    end
    ...
    return best_solution, best_cost, stats_c, stats_d, cumulative_destruction_outcomes, cumulative_construction_outcomes, tracker, accepted_costs, best_costs, temperature_evolution, iterations
end
...
# Plot temperature evolution with iterations
plot_temperature_evolution(temperature_evolution)
savefig("temperature_evolution_plot.png")
=#
