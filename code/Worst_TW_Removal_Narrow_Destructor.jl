"""
Worst Removal TW
Removes nodes from a route based on the width of their time windows, favoring those with narrower windows for more scheduling flexibility.
...
function Worst_Removal_Narrow_TW(solution::Vector{Int64}, percentage_to_remove::Float64, ei::Vector{Int64}, li::Vector{Int64})

    #println("we use worst removal Narrow TW")
    ...
    #println("Sorted nodes by TW Widths are (large): ", sorted_nodes_by_tw)

    # Extract only the nodes sorted by time window width
    sorted_nodes = [x[1] for x in sorted_nodes_by_tw]
    #println("Sorted nodes by TW Widths are (small): ", sorted_nodes)
...
remaining_solution, removed_nodes = worst_removal_tw(route, 0.3, ei, li)
