...
function Regret_Insertion_k2(
    ...
    while !isempty(removed_nodes)
        ...
        best_costs = Vector{BestCosts2}(undef, length(removed_nodes))  # undefined vector to store BestCosts2 structs
        ...
        for (idx, node) in enumerate(removed_nodes)     # if:   removed_nodes = [73, 47, 21, 42]
            ...
            # get best and second-best costs **for the current node**
            best_cost, best_position = sorted_costs[1]
            second_best_cost = sorted_costs[2][1]       # if sorted_costs = [(5.0, 4), (10.0, 17), (20.0, 9)]
                                                        # then sorted_costs[2] = (10.0, 17)
                                                        # and sorted_costs[2][1] = 10.0

            # store the best and second-best costs **for the current node**
            best_costs[idx] = BestCosts2(best_cost, second_best_cost, best_position)   # best_costs = [BestCosts2(10.0, 20.0, 3), BestCosts2(15.0, 28.0, 4), ...]
                                                                                      #                 for node 73 at idx=1 ,    for node 47 at idx=2 , ...
                                                                                      # idx = 1 --> (1, 73)
                                                                    
        end # now that the loop is over we have for all nodes to be reinserted the (best_cost, second_best_cost, best_position)
            # so that means best_costs[1] = BestCosts2(best_cost for node 73 (idx in removed_nodes = 1), second_best_cost for node 73 (idx in removed_nodes = 1), best_position to reinsert it)
        ...
        for (idx, node) in enumerate(removed_nodes)
            costs = best_costs[idx]                      # costs: an instance of the BestCosts2 struct 
                                                         # costs = best_costs[idx=1] = BestCosts2(10.0, 20.0, 3)
                                                         # costs.best_cost = 10.0
                                                         # costs.second_best_cost = 20.0
            regret_cost = costs.second_best_cost - costs.best_cost

            if regret_cost > max_regret
                max_regret = regret_cost
                best_node = node
                best_position = costs.best_position
                #println("CURRENTLY Max regret-2 cost is: ", max_regret, " for node: ", best_node, " and position: ", best_position)
            end
        ...
        #println(" k = 2: We ended up insering node: ", best_node, " at position: ", best_position)
...
new_solution, early_violations, early_violations_duration, early_violation_node_indices, late_violations, late_violations_duration, late_violation_node_indices, violations, violationsEarly, violationsLate, total_waiting_time = Regret_Insertion_k2(route, removed_nodes, d, t, h, ei, li, q)
=#